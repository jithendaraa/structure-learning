...
then
    if [ ${dataset} == 'er' ]
    then
        output_file="out/Decoder_BCD/graphvae-%A_%a.out"
        echo "Train Graph VAE ER: ${config}"
    else
...
command="sbatch --array=1-${array_len}%512 --job-name ${config} --output ${output_file} --time ${time} scripts/graphvae_job.sh ${config}"   
echo ""
echo ${command}
echo ""
