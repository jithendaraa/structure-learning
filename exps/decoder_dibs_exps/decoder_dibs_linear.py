...
def run_linear_decoder_dibs(opt, target, dag_file, rng, model, x, z_gt, interv_targets, p_z_mu, p_z_covar, writer):
    ...
    dibs_update = 0 
    intervs = jnp.unique(interv_targets, axis=0)
    ...
    @jit
    def train_causal_vars(state, key, z, theta, sf_baseline, data, interv_targets, step):
        recons, particles, q_z_mus, q_z_covars, dibs_grads, gs, sf_baseline, pred_zs = dibs.apply({'params': state.params}, 
    ...
    for step in range(opt.steps):
        if step < decoder_train_steps:
            state, particles, loss, mse_loss, kl_z_loss, q_z_mus, q_z_covars, gs, _, z_dist, q_z, particles = train_causal_vars(state, rng, z_final, theta_final, 
                                                                                                                sf_baseline, None, interv_targets, step)
            ...
            if (step+1) % 100 == 0:
                if opt.supervised is True:
                    ...
                    print(f"Step: {step} | Loss: {np.array(loss)} | MSE: {np.array(mse_loss)} | KL: {np.array(kl_z_loss)}")
                else: 
                    print(f"Step: {step} | MSE: {np.array(mse_loss)}")
            ...
            if (step+1) % 50 == 0:
                evaluate(target, joint_dibs_model, gs, theta_final, step - decoder_train_steps, dag_file, writer, opt, z_gt, interv_targets, True)
...
def run_linear_decoder_dibs_across_interv(opt, target, n_interv_sets, dag_file, rng, model, x, z_gt, no_interv_targets, p_z_mu, p_z_covar, writer):
    ...
    for i in range(n_interv_sets + 1):
        ...
        if opt.reinit is True:  
            target, model = get_target(rng, opt)
            dibs = Decoder_JointDiBS(opt.num_nodes, opt.num_samples, opt.proj_dims, 
                            opt.n_particles, model, opt.alpha_linear, dibs_type=opt.likelihood, 
                            latent_prior_std = 1.0 / jnp.sqrt(opt.num_nodes),
                            grad_estimator=opt.grad_estimator, known_ED=opt.known_ED, 
        ...
        def train_causal_vars(state, key, z, theta, sf_baseline, data, interv_targets, step):
            recons, particles, q_z_mus, q_z_covars, dibs_grads, gs, sf_baseline, pred_zs = dibs.apply({'params': state.params}, 
                                                                                                ...
                                                                                                data, interv_targets, step, 'linear')
            grads = grad(loss_fns.loss_fn)(state.params, key, z, theta, sf_baseline, data, interv_targets, 
                        step, data_gt, p_z_covar, p_z_mu, q_z_covars, q_z_mus, opt, dibs, 'linear')
            res = state.apply_gradients(grads=grads)
        ...
        @jit
        def train_causal_graph(state, key, z, theta, sf_baseline, data, interv_targets, step, opt_states):
            recons, _, q_z_mus, q_z_covars, dibs_grads, gs, sf_baseline, q_zs = dibs.apply({'params': state.params}, 
                                                                                            key, z, theta, sf_baseline, 
                                                                                            data, interv_targets, step, 'linear')
            opt_state_z, opt_state_theta = opt_states
            opt_state_z = opt_update(step, dibs_grads['phi_z'], opt_state_z)
            opt_state_theta = opt_update(step, dibs_grads['phi_theta'], opt_state_theta)
            return opt_state_z, opt_state_theta, gs, sf_baseline, q_zs

        for step in range(opt.steps):
            if step < decoder_train_steps:
                ( state, particles, loss, mse_loss, kl_z_loss, 
                    q_z_mus, q_z_covars, gs, _, z_dist, q_z, 
                    particles ) = train_causal_vars(state, rng, z_final, theta_final, 
                                                sf_baseline, data=None, 
                                                interv_targets=interv_targets, step=step)
                ...
                if step == decoder_train_steps:
                    ...
                    data = jnp.array(np.array(q_z)) # remove gradients on q_z
                t = dibs_update
                opt_state_z, opt_state_theta, gs, sf_baseline, q_zs = train_causal_graph(state, rng, z_final, theta_final, sf_baseline, 
                                                                    data, interv_targets, t, (opt_state_z, opt_state_theta))
                z_final, theta_final = get_params(opt_state_z), get_params(opt_state_theta)
                dibs_update += 1    
        ...
        if i > 0:
            _, _, _, obs_z_dist = loss_fns.calc_loss(None, None, None, None, None, None, q_zs[:, :opt.obs_data, ...], opt, 
                                    z_gt[ : opt.obs_data], only_z=True)

            _, _, _, interv_z_dist = loss_fns.calc_loss(None, None, None, None, None, None, q_zs[:, opt.obs_data: opt.obs_data + i*interv_data_per_set , ...], opt, 
