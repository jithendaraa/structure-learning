...
def train_batch(b, model_params, L_params, rng_key, 
    ...
    (rng_key, 
        ...
        z_samples,
        X_recons, 
        pred_W_means) = gradient_step(model_params, 
                                L_params,   
                                rng_key, 
    ...
    return ( rng_key, model_params, L_params, model_opt_params, L_opt_params,     
...
with tqdm(range(opt.num_steps)) as pbar:
    for i in pbar:
        ...
        if (i+1) % 50 == 0 or i == 0:    
            random_idxs = onp.random.choice(n_data, min(4000, n_data), replace=False)  
            ...
            wandb_dict = {
                "ELBO": onp.array(epoch_dict['ELBO']),
                "Z_MSE": onp.array(epoch_dict["z_mse"]),
            ...
            if opt.use_proxy:
                wandb_dict['proxy_obs_KL_term_Z'] = onp.array(epoch_dict["proxy_obs_KL_term_Z"])

            shd = mean_dict["shd"]
            tqdm.write(f"Step {i} | {epoch_dict['ELBO']}")
