...
def set_tb_logdir(opt):
  ...
  if opt.model in ['VCN', 'VAEVCN']:
    ...
    logdir += f'_({opt.num_nodes})_seed{opt.data_seed}_decoder_{opt.decoder_layers}_loss_{opt.train_loss}_L_KL{opt.L_KL}_interv_Z_KL{opt.interv_Z_KL}_Z_KL{opt.Z_KL}_learn_L{opt.learn_L}_pred_last_L{opt.pred_last_L}'
