...
def getfilename(args, output=False):
    ...
    cudnn_tag = args.cudnn_version
    py_tag = args.python_version
    if vercmp(jx_tag, "0.1.49") <= 0:
        lx_tag = "linux"
    elif vercmp(jx_tag, "0.3.10") >= 0:
        lx_tag = "manylinux2014"
    else:
        lx_tag = "manylinux2010"
    lx_tag = (output and args.tag) or lx_tag
    return f"jaxlib-{jx_tag}+{cu_tag}{cudnn_tag}-{py_tag}-none-{lx_tag}_x86_64.whl"
...
if __name__ == "__main__":
    ...
    argp.add_argument("--jaxlib-version", "-V", default="0.3.15")
    argp.add_argument("--cuda-version",   "-C", default="cuda11")
    argp.add_argument("--python-version", "-P", default="cp38")
    argp.add_argument("--cudnn_version", "-cudnn", default=".cudnn82")
