...
def register_my_chemistry_rl_envs(name = 'LinGaussColorCubesRL-{}-{}-{}-{}-{}', num_objects = 5,
                            num_colors = 5, movement = 'Dynamic', max_steps = 50, **kwargs):
    register(
        name.format(num_objects, num_colors, movement, max_steps, 'v0'),
        entry_point = 'envs.my_chem_env_rl:LinGaussColorCubesRL',
        kwargs=dict(
            render_type='shapes',
            num_objects=num_objects,
            num_colors=num_colors,
...
for n_obj in [3, 4, 5, 6, 7, 8, 9, 10]:
    for n_colors in [3, 4, 5, 6, 7, 8, 9, 10]:
...
for n_obj in [3, 4, 5, 6, 7, 8, 9, 10]:
    for n_colors in [3, 4, 5, 6, 7, 8, 9, 10]:
        for movement in ['Static']:
            for steps in [10, 20, 30, 40, 50, 200]:
                register_my_chemistry_rl_envs('LinGaussColorCubesRL-{}-{}-{}-{}-{}', n_obj, n_colors, movement, steps)
