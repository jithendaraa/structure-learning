...
def set_tb_logdir(opt):
  ...
  if opt.model in ['VCN', 'VAEVCN']:
    ...
    logdir += f'_({opt.num_nodes})_seed{opt.seed}_{opt.data_seed}_proj{opt.proj}{opt.proj_dims}_samples{opt.num_samples}_expedges{opt.exp_edges}_sftcnstrnt_{opt.soft_constraint}_steps{opt.steps}_knownED{opt.known_ED}'
